...
| 스킬 | 호출 | 설명 |
|------|------|------|
| **start-workflow** | `/fe-harness:start-workflow` | 전체 프론트 워크플로우 자동화 — 요청→난이도·검증 티어 판정→Plan 리뷰→구현→품질 루프→PR. light 티어면 리뷰 레이어·루프 상한·E2E 범위 자동 축소(`--tier standard`로 상향 강제), 성찰은 `--reflect` 시에만, 종료 시 md Workflow Report 아카이브 |
