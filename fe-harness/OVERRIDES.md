...
플러그인 원본 파일(`fe-harness/skills/{name}/SKILL.md`)은 **절대 수정하지 않는다**. 프로젝트 특화 규칙은 아래 경로의 오버라이드 파일에만 작성한다.
...
1. **플러그인 기본 동작** — `fe-harness/skills/{name}/SKILL.md` 또는 `fe-harness/agents/{name}.md` (Claude Code가 로드)
2. **공통 오버라이드** — `.claude/fe-harness/common.md` 가 있으면 먼저 읽는다
3. **스킬별 오버라이드** — `.claude/fe-harness/skills/{name}.md` 또는 `.claude/fe-harness/agents/{name}.md` 가 있으면 읽는다
...
- 적용 범위: **현 프로젝트에만**
- 경로: `.claude/fe-harness/{skills,agents,common}/{name}.md`
- 플러그인 원본은 건드리지 않음
- 파일이 없으면 생성
...
- 해당 프로젝트에서는 컴포넌트에 data-testid 필수
- E2E 시작 전 mock API 부팅을 `pnpm mock` 으로 고정
```
...
| 구분 | 용도 | 파일 경로 |
|------|------|----------|
| profile | 빌드/테스트 명령, 디렉토리 경로 등 **값(setting)** | `.claude/fe-harness.local.md` |
| projectConventions | `convention-check` 가 검사 기준으로 삼는 컨벤션 문서 | profile의 `projectConventions` 필드 (예: `CLAUDE.md`) |
| **Project Overrides (이 문서)** | **각 스킬/에이전트에 대한 프로젝트별 동작 조정/추가 규칙** | `.claude/fe-harness/{skills,agents}/{name}.md` |
...
1. 스킬 실행이 시작되면 아래 순서로 Read:
   a. .claude/fe-harness/common.md (있으면)
   b. .claude/fe-harness/skills/{본인 스킬 이름}.md (있으면)
2. 위 내용을 "프로젝트 추가 지시"로 해석하여 스킬 본문 흐름에 반영
3. 각 단계가 오버라이드에 의해 수정/추가/제거되었는지 판단
4. 최종 결과 보고 시 어떤 오버라이드가 적용되었는지 명시
```

에이전트도 동일 — 본인 프롬프트 실행 전에 `.claude/fe-harness/agents/{name}.md` 를 Read.
