...
```bash
git rev-parse HEAD
{testCommand}       # 비어있으면 {testRunner} 기반 fallback (vitest run / jest)
{e2eCommand}        # e2eRunner 가 none 이 아니면 별도 suite로 수집
```
...
| `정규화 시그니처` | 실패 메시지 첫 줄에서 **경로·라인 번호·타임스탬프·스냅샷 해시를 제거**한 문자열 |
...
Phase 7.1(build+typecheck)과 7.4(test-loop) 결과를 `## Test Baseline`과 대조해 분류한다.
