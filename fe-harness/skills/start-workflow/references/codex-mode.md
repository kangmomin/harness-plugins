...
- `max`의 5.1: Codex `write` 슬롯이 `{PLUGIN_ROOT}/skills/unit-test/SKILL.md` 절차를 직접 읽어 수행한다 (`references/tdd.md`). none·mix는 기존 Skill tool 경로.
- 레거시 profile(`.hyeondong-config.json`)만 있는 프로젝트는 §2의 ephemeral 규칙 — `codexMode`·`codexModels`를 저장하려면 `/fe-harness:init`으로 `.claude/fe-harness.local.md`를 생성한다.
- 오버라이드 경로: `{CWD}/.claude/fe-harness/common.md` · `{CWD}/.claude/fe-harness/agents/{name}.md`.
