...
|------|------|------|
| Plan 검증 루프 | Phase 3.3 (난이도 = Phase 2 종합 난이도) | — (Spec·Plan 전문 전달) |
| 특화 하네스 품질 리뷰 | 해당 없음 (hyeondongs 오버레이는 리뷰 단계를 삽입하지 않음) | — |
| 탐색·수집 / 이해·요약 | 탐색 위임 에이전트 · 7.7 Read-back 복원(general-purpose) | — (general-purpose) |
| 읽기 전용 판정 | 7.5 `scope-reviewer` · 8 `component-reviewer` · 8 `a11y-reviewer` | `{PLUGIN_ROOT}/agents/{name}.md` |
| 쓰기 | 5.1 Red(러너 내부 리프) · 5.2 `workflow-implementer` · 6 build-fix · 7.2/7.3/7.4/7.6 수정(러너 내부 리프) · 8 Critical 이슈 수정 · 10 `workflow-reflection` | `{PLUGIN_ROOT}/agents/{name}.md` (general-purpose는 생략) |
...
- `max`의 5.1: Codex sol이 `{PLUGIN_ROOT}/skills/unit-test/SKILL.md` 절차를 직접 읽어 수행한다 (`references/tdd.md`). none·mix는 기존 Skill tool 경로.
- 레거시 profile(`.hyeondong-config.json`)만 있는 프로젝트는 §2의 ephemeral 규칙 — `codexMode`를 저장하려면 `/fe-harness:init`으로 `.claude/fe-harness.local.md`를 생성한다.
