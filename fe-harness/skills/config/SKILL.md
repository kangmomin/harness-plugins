...
- `.claude/fe-harness.local.md`만 수정한다. 본문·오버라이드·레거시 JSON·워크플로우 상태·Codex 인증 파일은 수정하지 않는다.
...
`PROFILE.md`의 키·허용값이 사용자 계약이고 helper `schema --domain fe`이 실행 검증을 담당한다. 두 집합은 CI에서 일치 여부를 검사한다.
...
```bash
python3 -I -B "{PLUGIN_ROOT}/skills/config/assets/profile.py" resolve --domain fe --cwd "{CWD}"
```
...
```bash
python3 -I -B "{PLUGIN_ROOT}/skills/config/assets/profile.py" edit --domain fe --cwd "{CWD}" <<'HARNESS_PROFILE_JSON'
