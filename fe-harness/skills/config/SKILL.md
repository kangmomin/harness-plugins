---
name: config
description: "fe-harness project profile(.claude/fe-harness.local.md)의 설정 값을 조회하고 키 단위로 수정한다. '프로필 설정 확인해줘', '설정 값 바꿔줘', '{키} 값 뭐야', '{키}를 {값}으로 바꿔줘' 요청 시, init 재실행 없이 값 하나만 보거나 고칠 때 사용. 파일 생성·환경 진단은 하지 않는다 (init·doctor 담당). 레거시 .hyeondong-config.json만 있으면 조회만."
allowed-tools: Read, Edit, AskUserQuestion
user-invocable: true
argument-hint: "[{키} | {키}={값} …]"
---

> **Project Overrides**: 실행 전 `.claude/fe-harness/common.md`와 `.claude/fe-harness/skills/config.md`를 Read.
...
profile(`.claude/fe-harness.local.md`)의 설정 값을 **조회**하고 `{키}={값}` 배치로 **수정**한다. `init`을 다시 돌리지 않고 값 하나를 보거나 바꾸는 경로다. 레거시 profile(`.hyeondong-config.json`)만 있으면 조회만 한다.

호출: `/fe-harness:config` (전체 조회) · `/fe-harness:config {키}` (단일 조회) · `/fe-harness:config {키}={값} [{키}={값} …]` (배치 수정)

## 원칙

- 파일을 **생성하지 않는다** — 없으면 `/fe-harness:init` 안내. 명령 실행·자동 감지·환경 점검도 하지 않는다 (`doctor` 담당).
- **쓰는 파일은 `{PROFILE}` 하나뿐이다.** 읽는 파일은 `{PROFILE}`·`{LEGACY}`(읽기만)·`{PLUGIN_ROOT}/PROFILE.md`·codex-mode.md §2.1(`codexModels` 관련 시)·머리말의 Project Overrides 파일뿐이며 Overrides는 편집하지 않는다. `~/.codex/config.toml`(시크릿)·start-workflow 상태 파일은 읽지도 쓰지도 출력하지도 않는다.
...
- 플레이스홀더(본문에서는 이 이름만 사용):
  ...
  - `{PROFILE}` = `.claude/fe-harness.local.md`
  - `{LEGACY}` = `.hyeondong-config.json` (읽기 전용 2순위 profile)
...
| 항목 | 미충족 시 |
|------|----------|
| `{PROFILE}` 또는 `{LEGACY}` 존재 | 둘 다 없음 → `BLOCKED:NO_PROFILE` — "1. `/fe-harness:init`으로 profile 생성(사용자) 2. 종료" |
| `{PLUGIN_ROOT}/PROFILE.md` (스키마 canonical — 키별 허용값·빈 값 의미·레거시 매핑 표·쓰기 규칙) | Step 1에서 MUST Read. 읽기 실패 → `FAIL` (플러그인 설치 이상) |
...
키 분류는 아래 마커 안이 canonical이며 `PROFILE.md` 프론트매터의 키 집합과 양방향으로 일치한다(레포 가드가 검사). 허용값·빈 값의 의미는 `PROFILE.md`를 따른다(구현 시 init 선택지와 동일).

<!-- config:keys-begin — scripts/check-plugins.sh §7 parity 대상 -->
| 타입 | 키 | 값 규칙 |
|------|----|--------|
| enum | `preset` (node) · `language` (ko \| en) · `codexMode` (none \| mix \| max) · `testRunner` (vitest \| jest) · `e2eRunner` (playwright \| cypress \| none) · `packageManager` (pnpm \| yarn \| npm \| bun) · `componentPattern` (feature-based \| atomic \| flat) | trim 후 exact — 빈 값 무효 |
| bool | `typescript` `storybook` | true \| false exact |
| string | `framework` `uiLibrary` `stateManagement` `buildCommand` `testCommand` `lintCommand` `typeCheckCommand` `e2eCommand` `runServerCommand` `serverUrl` `e2eLockDir` `reportDir` `mainBranch` `featureBranchPrefix` `hotfixBranchPrefix` `commitCoAuthor` | 자유 문자열 — 빈 문자열 유효 (framework·uiLibrary·stateManagement는 PROFILE.md 주석의 목록 권장, init "기타" 허용) |
...
그 외 키: `미설정` — 명령류(`buildCommand` 등)·`packageManager`는 비고 "실행 시 자동 감지"(PROFILE.md 읽기 우선순위 3·4).

## Step 1: 로드·구조 판정

1. `{PLUGIN_ROOT}/PROFILE.md`를 Read한다 (MUST — 키별 허용값·빈 값 의미·레거시 매핑 표·쓰기 규칙의 canonical). 인자 원문 또는 profile에 `codexModels`가 있으면 `{PLUGIN_ROOT}/skills/start-workflow/references/codex-mode.md` §2.1도 MUST Read한다 (레코드 패턴·effort enum — Step 2·3 검증과 조회의 `⚠ INVALID_SLOT` 판정에 필요).
2. `{PROFILE}`을 Read한다. 없으면 `{LEGACY}`를 확인한다:
   - `{LEGACY}` 있음 → **레거시 조회 모드**. JSON을 파싱해 PROFILE.md 필드 매핑 표를 적용한다(같은 키는 그대로, `conventions[].source = project`의 `path`만 모아 `projectConventions`로 — plugin 항목뿐이면 빈 배열, 그 외 미매핑 키는 `미설정`). 파싱 실패 → `BLOCKED:INVALID_PROFILE` + "1. `/fe-harness:init`으로 profile 생성(사용자) 2. 종료". 레거시 전용 키(`conventions`·`name` 등)는 표시하지 않는다(PROFILE.md: 무시). 조회 헤더의 경로는 `{LEGACY}`. 아래 3~5는 `{PROFILE}`에만 적용한다. 이 모드에서 수정 요청(Step 2의 배치 수정)은 `BLOCKED:LEGACY_READONLY`로 종료하며 PROFILE.md "쓰기 규칙" 문구를 그대로 안내한다:
     > "`.hyeondong-config.json` 은 읽기 전용입니다. 설정을 갱신하려면 `/fe-harness:init` 으로 `.claude/fe-harness.local.md` 를 생성하세요 (기존 값이 기본값으로 채워집니다)."
   - 둘 다 없음 → `BLOCKED:NO_PROFILE` + 선택지 "1. `/fe-harness:init`으로 profile 생성(사용자) 2. 종료".
3. **EOL·구분선**: EOL은 각 줄의 LF 또는 CRLF(줄별, 혼합 허용). 1행이 `---`(뒤 공백·탭 허용) + EOL이고, 그 다음으로 `---`(뒤 공백·탭 허용) + EOL **또는 파일 끝**이 처음 나오는 줄이 닫는 구분선이다. 본문(닫는 구분선 뒤)은 불투명 — 본문의 `---`는 무시. 여는/닫는 구분선이 없거나 루트 키가 중복되면 `BLOCKED:INVALID_PROFILE`(전역 — 수정은 전부 차단. 조회: 닫는 구분선이 없으면 표 없이 종료, 루트 키 중복이면 그 키 행의 값 대신 `⚠ 중복 키(N회)` 표시. 선택지: "1. 직접 편집 후 재호출 2. `/fe-harness:init` 실행(사용자)").
...
`$ARGUMENTS` 원문을 스캐너로 항목 분리 → 디코딩 → 모드 판정. 레거시 조회 모드에서 배치 수정이면 Step 3 진입 없이 `BLOCKED:LEGACY_READONLY`(Step 1 문구).

| 입력 | 모드 |
|------|------|
| (없음) | 전체 조회 → (대화형, writable profile) 수정 입력 질문 / (비대화형·레거시) 조회만 `DONE` |
| 항목 1개, `=` 없음 | 단일 조회 |
| 모든 항목이 `{키}={값}` | 배치 수정 |
...
- **인자 없음**: 조회 표 출력 → (비대화형·레거시) `DONE` / (대화형) Q1:
...
| 재입력도 무효 (재입력은 1회뿐) | `BLOCKED:INVALID_VALUE` — "1. 올바른 값으로 재호출 2. `/fe-harness:init`" |
...
하나라도 있으면 배치 전체 차단(파일 불변) + 해당 줄 인용 + 선택지 "1. 직접 편집 후 재호출 2. `/fe-harness:init` 실행(사용자)".
...
수정 결과 표(`키 | 이전 | 이후 | 상태` — 이전/이후 셀은 조회 셀 규칙 그대로, 부재 키는 `(없음)`) — 배치는 전부 `DONE` 또는 전부 `BLOCKED`/`FAIL`. 변경 0건(동일 값) → "변경 없음" `DONE`. 끝에 "`/fe-harness:doctor`로 확인" 1줄. `codexMode`/`codexModels`를 바꿨으면 고지: "진행 중·재개되는 워크플로우는 상태 파일 값을 유지하며 새 값은 다음 실행부터 적용".

## 출력 형식

조회(전체/단일 — 레거시 모드는 헤더에 `(레거시 · 읽기 전용)`, 마지막 줄을 "변경 불가 — `/fe-harness:init`으로 마이그레이션"으로):

```markdown
## fe-harness Config — `.claude/fe-harness.local.md`
| # | 키 | 값 | 출처 | 비고 |
|---|----|----|------|------|
| 1 | preset | node | profile | |
| 2 | e2eRunner | playwright | profile | |
| 3 | buildCommand | (없음) | 미설정 | 실행 시 자동 감지 |
| 4 | codexModels | (기본) | 기본값 | 4슬롯 기본 — 상세: `config codexModels` · codexMode none이면 N/A |
| 5 | reportDir | (없음) → .claude/harness-reports | 기본값 | |
| — | ⚠ fooBar | x | 알 수 없는 키 | 어떤 스킬도 읽지 않음 |
변경: `/fe-harness:config {키}={값} …` · 파일 생성/전체 재설정: `/fe-harness:init`
```

- 출처: `profile`(파일 명시 — 빈 문자열도 profile; 비고 = PROFILE.md 해당 키 주석의 빈 값 의미) / `기본값`(문서 기본값 표) / `레거시`(`{LEGACY}` 매핑 값) / `미설정`(그 외 — 명령류·packageManager는 비고 "실행 시 자동 감지").
...
```markdown
## fe-harness Config — 수정 결과
| 키 | 이전 | 이후 | 상태 |
|----|------|------|------|
| e2eRunner | playwright | none | DONE |
| codexModels | (기본) | (1슬롯 설정) review=zai/glm-5.3@high | DONE |
`/fe-harness:doctor`로 확인. 진행 중·재개되는 워크플로우는 상태 파일 값을 유지하며 새 값은 다음 실행부터 적용.
```
...
| `BLOCKED:NO_PROFILE` | `{PROFILE}`·`{LEGACY}` 모두 없음 → `init` 안내 |
| `BLOCKED:LEGACY_READONLY` | 레거시 profile만 있을 때 수정 요청 |
| `BLOCKED:INVALID_PROFILE` | 구분선 없음·루트 키 중복(전역) / 레거시 JSON 파싱 실패 / 저장된 codexModels 블록 무효(키 범위) |
...
- `{PLUGIN_ROOT}/PROFILE.md` — Step 1 MUST Read: 키별 허용값·빈 값 의미·읽기 우선순위·레거시 필드 매핑·쓰기 규칙.
