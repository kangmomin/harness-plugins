...
> **Project Overrides**: 실행 전 `.claude/fe-harness/common.md`와 `.claude/fe-harness/skills/simplify-loop.md`를 Read.
...
Workflow 경로의 3/4 찬성 후보는 실제 소수 의견으로 Arbiter에 직접 보낸다. 이 경로의 모든 승인에 반론 해소 여부와 코드·실행된 테스트 근거가 필요하다. 구체적 동작 위험을 찬성표 수로 넘기지 않으며, 미해소·근거 누락은 HOLD, 잘못된 중재 결과는 기존 인프라 재시도로 처리한다.
