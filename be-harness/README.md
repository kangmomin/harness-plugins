...
| 스킬 | 호출 | 설명 |
|------|------|------|
| **start-workflow** | `/be-harness:start-workflow` | 전체 워크플로우 자동화 — 요청→난이도·검증 티어 판정→Plan 리뷰→구현→품질 루프→PR. 저위험·저복잡도(light)면 리뷰 레이어·루프 상한·E2E 범위를 자동 축소(`--tier standard`로 상향 강제), 성찰은 `--reflect` 시에만, 종료 시 md Workflow Report 아카이브 |
...
| **e2e-test** | `/be-harness:e2e-test` | profile 기반 HTTP API E2E 테스트. `--smoke`는 Spec 유래 시나리오(BASE-01 + EC-*)만 |
| **e2e-test-loop** | `/be-harness:e2e-test-loop` | E2E → 수정 → 재테스트 반복 (최대 5회, `--smoke` 3회). 종료 시 정직한 자기 점검 md 리포트 생성 (`render_e2e_report.py`) |
