...
기대 status·응답·금지 동작은 작업 계약의 요구·기준 문서에서 정한다. 아래 기본값과 다르면 계약상의 이유를 적고, 실제 응답에 맞추어 기대값을 낮추지 않는다.
...
권한·소유권 조건은 Spec의 일반 사용자 성공·미인증·권한 없음·다른 소유자 케이스를 각각 실행한다. Spec에 필요한 조건이 빠졌다면 누락을 호출자에게 돌려주고 임의 기대값으로 EC를 만들지 않는다. Spec EC는 `--smoke`에서도 생략하지 않는다.
