...
| — | Spec 자체가 제공되지 않음 (단독 호출) | 공통 scope의 변경 함수의 **공개 계약** | `추적 기준 없음` |
...
이번 Spec으로 **기대 동작이 실제로 바뀐** 테스트만 수정한다. 그 외 기존 테스트는 읽기만 한다. 실패하면 요구·기준 문서와 대조해 구현 오류 / 테스트 오류 / 환경 문제를 구분하고, 기대값을 현재 응답에 맞추지 않는다. 테스트 자체 오류의 근거는 호출자에게 반환하여 기존 `[TestConflict]`·수정 권한 계약으로 처리한다.
