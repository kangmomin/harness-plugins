...
description: "프로젝트 워크플로우에서 수집된 be-harness 피드백을 플러그인 레포(kangmomin/harness-plugins)의 community-feedback 영역에 PR로 제출한다. 실패 시 로컬 저장으로 fallback."
allowed-tools: Read, Write, Edit, Glob, Grep, Bash, AskUserQuestion
argument-hint: "<보완점 항목 JSON> 또는 대화 컨텍스트에서 수집"
user-invocable: true
---

> **Project Overrides**: 실행 전 `.claude/be-harness/common.md`와 `.claude/be-harness/skills/submit-feedback.md`를 Read.
> 존재하면 추가 규칙/예외로 흡수하고 충돌 시 오버라이드가 우선한다. 상세 규약: 플러그인 루트 `OVERRIDES.md`.

# submit-feedback — 피드백을 플러그인 레포에 PR로 제출

`start-workflow` Phase 12 에서 수집된 **범용성 있는 보완점**을 플러그인 원본 레포의 `community-feedback/` 영역에 PR로 보낸다.

## 언제 쓰나

- 보완점이 "이 프로젝트만의 규칙"이 아니라 **다른 팀에도 도움이 될 범용 규칙**이라고 판단될 때
- 유저가 Phase 12 에서 "로컬 저장 + PR" 옵션을 선택했을 때 자동 호출됨
- 수동으로 단독 실행도 가능 (`/be-harness:submit-feedback`)

## 언제 쓰지 말아야 하나

- 피드백이 프로젝트 특정 도메인/스택에만 의미 있을 때 → 로컬 오버라이드(`.claude/be-harness/...`)만 쓰고 여기 오지 않음
...
| 플러그인 `upstream.repo` | 필수 | `be-harness/.claude-plugin/plugin.json` 읽기 |
| 네트워크 접근 | 필수 | 시도 후 판정 |
...
- 모드: {self-owner / fork}
- 추가할 파일:
  - be-harness/community-feedback/skills/start-workflow.md (+1 엔트리)
  - be-harness/community-feedback/agents/workflow-implementer.md (+1 엔트리)
- 브랜치명: feedback/be-harness/{YYYYMMDD}-{요약-kebab}
- PR 제목: "[be-harness] feedback: {요약}"
...
**출처**: /be-harness:start-workflow Phase 11 성찰
...
```bash
BRANCH="feedback/be-harness/$(date +%Y%m%d)-{요약-kebab}"
git checkout -b "$BRANCH" 2>/dev/null || git checkout "$BRANCH"

# 각 feedback_item 에 대해 해당 파일에 append (없으면 생성)
# 파일 경로: be-harness/community-feedback/{skills|agents|common}/{name}.md
...
```bash
git add be-harness/community-feedback/
git commit -m "Add: be-harness 커뮤니티 피드백 추가: {요약}"
```
...
```markdown
## Summary
be-harness 사용 중 수집된 피드백 {N}건을 community-feedback 영역에 추가합니다.
...
## 출처
- 워크플로우: /be-harness:start-workflow Phase 11 성찰 (자동 제출)

## 주의
- 원본 SKILL.md 는 변경하지 않습니다.
- 유지보수자 검토 후 범용 규칙으로 승격 여부를 결정합니다.

자동 생성: /be-harness:submit-feedback
```
...
fallback 시 호출자(`start-workflow`)는 로컬 저장(`.claude/be-harness/...`)만 수행하고 정상 종료. 워크플로우 전체를 실패로 만들지 않는다.
...
> - Yes → PR 대상
> - No → 로컬 전용 (.claude/be-harness/... 에만 저장, PR 생략)
