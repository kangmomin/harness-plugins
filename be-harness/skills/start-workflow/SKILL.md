...
| 종료 조건 | 결과 |
|----------|------|
| iteration 내 수정 0건 (`modified == false`) | 루프 탈출 → Phase 9 |
| `modified == true` | 커밋 후 다음 iteration |
| 3회 도달 | 미해결 사항 보고 후 강제 탈출 → Phase 9 |

커밋: `git add [수정 파일들] && git commit -m "Fix: 품질 루프 수정 (반복 N)"`
완료 후: "Phase 8 완료: [루프 횟수]회, 총 [수정 건수]건 수정"
...
- `$HARD_MODE = true`: PR 생략, `git push origin $(git branch --show-current)` 후
  "Phase 10 완료: `{브랜치명}`에 push 완료 (--hard 모드, PR 생략)" 출력.
...
2. 보완점 반영 방식을 질문한다: ① 로컬에만 저장 (기본) ② 로컬 저장 + `/be-harness:submit-feedback`으로 PR ③ 건너뛰기.
3. 적용 절차·append 규칙은 templates.md의 "보완점 적용 상세"를 따른다. 플러그인 원본은 절대 수정하지 않는다.
4. 정리: 상태 파일의 모든 Phase를 `DONE`/`SKIPPED:{사유}`로 갱신하고 `Remaining Phases`를 `없음`으로 기록.
