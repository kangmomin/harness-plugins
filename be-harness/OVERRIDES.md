...
플러그인 원본 파일(`be-harness/skills/{name}/SKILL.md`)은 **절대 수정하지 않는다**. 프로젝트 특화 규칙은 아래 경로의 오버라이드 파일에만 작성한다.
...
│   └── ... (스킬 이름과 동일하게)
└── agents/
    ├── workflow-implementer.md        # be-harness:workflow-implementer 에이전트 오버라이드
    ├── code-verifier.md
    └── ... (에이전트 이름과 동일하게)
```
...
1. **플러그인 기본 동작** — `be-harness/skills/{name}/SKILL.md` 또는 `be-harness/agents/{name}.md` (Claude Code가 로드)
2. **공통 오버라이드** — `.claude/be-harness/common.md` 가 있으면 먼저 읽는다
3. **스킬별 오버라이드** — `.claude/be-harness/skills/{name}.md` 또는 `.claude/be-harness/agents/{name}.md` 가 있으면 읽는다

### 충돌 해결

시스템·호스트 권한 안에서 사용자의 명시적 지시와 기존 승인은 아래 기본값/override 관계보다 우선한다. 일반 파일 지침만으로 이미 승인된 범위를 다시 승인받지 않는다. 플러그인 원본 수정 금지는 프로젝트 사용 시의 규칙이며, 사용자가 요청한 하네스 자체 유지보수에는 적용하지 않는다.
...
특화 하네스(`minmos-harness`)는 이 레이어를 **경로 B**로 사용한다. `/minmos-harness:init` 이 자기 `overlay/*.md` 를 위 경로에 복사하면, `/be-harness:*` 를 직접 호출해도 특화 규칙이 적용된다.

복사된 파일은 첫 줄에 `<!-- overlay-source: minmos-harness@{version} -->` 마커를 갖는다. 마커가 있는 파일은 오버레이 플러그인이 관리하므로 **수동 편집하지 않는다** (init 재실행 시 덮어써진다). 프로젝트 고유 규칙은 마커 없는 별도 파일에 쓴다 — 둘은 함께 적용된다.
...
| 구분 | 용도 | 파일 경로 |
|------|------|----------|
| profile | 빌드/테스트 명령, 디렉토리 경로 등 **값(setting)** | `.claude/be-harness.local.md` |
| projectConventions | `convention-check` 가 검사 기준으로 삼는 컨벤션 문서 | profile의 `projectConventions` 필드 (예: `CLAUDE.md`) |
| **Project Overrides (이 문서)** | **각 스킬/에이전트에 대한 프로젝트별 동작 조정/추가 규칙** | `.claude/be-harness/{skills,agents}/{name}.md` |
...
1. 스킬 실행이 시작되면 아래 순서로 Read:
   a. .claude/be-harness/common.md (있으면)
   b. .claude/be-harness/skills/{본인 스킬 이름}.md (있으면)
2. 위 내용을 "프로젝트 추가 지시"로 해석하여 스킬 본문 흐름에 반영
3. 각 단계가 오버라이드에 의해 수정/추가/제거되었는지 판단
4. 최종 결과 보고 시 어떤 오버라이드가 적용되었는지 명시
```

에이전트도 동일 — 본인 프롬프트 실행 전에 `.claude/be-harness/agents/{name}.md` 를 Read.
