...
| 항목 | 점검 방법 | 판정 |
|------|----------|------|
| 경로 A (플러그인 내장) | `${CLAUDE_PLUGIN_ROOT}/overlay/start-workflow.md` 존재 | 항상 OK (플러그인에 동봉) |
| 경로 B (프로젝트 복사) | `.claude/be-harness/skills/start-workflow.md` 의 `<!-- overlay-source: minmos-harness@ -->` 마커 | INSTALLED / NOT_INSTALLED / FOREIGN(마커 없는 사용자 오버라이드) |

- 경로 B가 `NOT_INSTALLED` 여도 정상이다 — `/minmos-harness:*` 로 호출하면 경로 A가 적용된다. `/be-harness:*` 를 직접 호출하는 팀만 `/minmos-harness:init` 의 오버레이 설치 단계를 실행하면 된다.
- `FOREIGN` 이면 사용자 작성 오버라이드와 공존 중이라는 뜻이다. 두 규칙이 함께 적용되므로 충돌 여부를 확인하도록 안내한다.
...
| # | 항목 | 해결 방법 |
|---|------|----------|
| 1 | [항목] | `/minmos-harness:init` 실행 또는 [구체적 안내] |
```
