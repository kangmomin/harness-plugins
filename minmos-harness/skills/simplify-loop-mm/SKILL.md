...
description: "변경 코드의 단순화 후보를 4관점 리뷰 Workflow 루프로 수렴할 때까지 반복 적용한다 (최대 10회, Workflow tool 미지원 시 빌트인 /simplify 반복 폴백). 구현 직후 코드 단순화 정리, '심플리파이 돌려줘', '코드 간소화' 요청 시 사용. start-workflow-mm 품질 루프(dry-run)와 start-workflow-fs Phase 8에서 자동 호출됨."
user-invocable: true
---
...
| 플래그 | 효과 |
|--------|------|
| `--dry-run` | 스캔 1회만 수행하고 후보 목록을 보고 (파일 수정 없음, Workflow 불필요). 호출 컨텍스트가 "dry-run 관점"을 지시하는 경우(예: start-workflow-mm Phase 9.2)도 이 플래그와 동일하게 해석한다 |
...
**3순위 — 직접 수행 절차** (단일 패스, 반복 없음 — 반복은 상위 호출자의 루프가 담당. 예: start-workflow-fs Phase 8은 자체 도메인 루프 최대 3회):
