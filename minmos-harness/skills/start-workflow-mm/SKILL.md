...
**Plan은 Codex가 APPROVE할 때까지 반복되는 검증 루프를 통과해야 확정된다.**
반복 횟수에 상한은 없다.
...
[Claude가 Plan 수정 → v(N+1)]   [Phase 3.4 Plan 확정 → Phase 3.5]
```
...
- **반영**: [수용한 Codex 피드백 요약]
- **기각**: [기각한 피드백 + 사유 (예: "범위 외", "비용 대비 낮음")]
- **Plan 변경 요약**: [vN → v(N+1) 핵심 diff]
```

#### 데드락 / 안전장치

- **동일 이슈 3회 반복 지적**: Codex가 같은 이슈를 3 iteration 연속 동일하게 지적하면 사용자에게 보고하고 판단을 위임한다. 사용자 응답 후 루프를 재개하거나 종료한다.
- **실질적 진전 확인**: Diff Log에 실제 Plan 변경이 0건인 iteration이 발생하면 즉시 중단하고 사용자에게 보고한다 (무한 핑퐁 방지).
...
상태 파일 하단에 `Plan Verification Summary`를 기록한다:
...
[Phase 3.2 검증 루프 Iteration Diff Log를 시간순으로 기록]
