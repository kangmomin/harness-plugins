...
description: "E2E 테스트 → 이슈 수정 → 재테스트 반복 (최대 5회). '테스트 통과할 때까지 고쳐줘', 'E2E 루프 돌려줘' 요청 시 사용. 종료 시 정직한 자기 점검 HTML 리포트 생성. start-workflow-mm 품질 루프에서 자동 호출됨."
user-invocable: true
---
...
**Probe 항목 (e2e-test-mm Step 1과 동일):**
...
> **MCP 판정**: 실제 PostgreSQL MCP 호출 성공 = 연결 OK. `.mcp.json` 존재 여부는 단독 기준으로 쓰지 않는다 (상세: `/minmos-harness:minmo-doctor-mm`).
...
- 하나라도 FAIL → **루프 진입 없이** 즉시 아래 형식으로 종료 (리포트 파일 생성·doc-gen 렌더링 없음):
  ...
  ```
- `--skip-doctor` / `-sd` 지정 시 → probe를 건너뛰고 바로 Step 2로 진행 (사용자 책임)

### Step 2: E2E 실행 리포트 초기화

> Probe를 통과해 **실제 루프에 진입하는 경우에만** 수행한다. `--init` / `--doctor` 분기, Step 1 SKIP 종료 경로에서는 리포트 파일을 만들지 않는다.

루프 동안 수행하는 모든 테스트 케이스의 요청 데이터·기대·실제·판정과 실패→수정 내역을 `/tmp/e2e-run-report.md`에 누적한다. 루프 종료 후 이 파일이 Step 4 HTML 렌더링의 입력이 된다.
...
## 테스트 대상 엔드포인트
<!-- Step 3 첫 iteration에서 e2e-test-mm가 도출한 엔드포인트 목록을 1회 채운다 -->
...
1. `/minmos-harness:e2e-test-mm` 를 실행한다.
   ...
   - 하위 스킬이 `SKIPPED:*`를 반환하면 루프를 추가 진행하지 않는다. 빈 리포트 파일을 삭제(`rm -f /tmp/e2e-run-report.md`)하고 동일 SKIP 사유로 "종료 시 출력"의 SKIPPED 형식으로 보고한다. **Step 4(HTML 렌더링)은 건너뛴다.**
   - 정상 실행되면, 이번 iteration의 **모든 테스트 케이스**(통과·실패 무관)를 Step 2의 "케이스 블록 형식"으로 `/tmp/e2e-run-report.md`의 `## Iteration 기록` 아래에 append 한다.
2. 결과를 확인한다:
   - **이슈 없음** (모든 테스트 통과, STATUS_MISMATCH 없음) → 루프를 종료하고 Step 4로 진행한다.
   - **미커버만 존재** (테스트 실패 0건 + `UNCOVERED:{사유}` 1건 이상 = e2e-test-mm 판정 `WARN`) → 루프를 종료하고 Step 4로 진행한다. 미커버는 검증 공백이지 구현 결함이 아니므로 수정 루프를 돌리지 않고, 사유를 리포트에 남긴 채 상위에 전달한다.
   - **이슈 발견** → 3번으로 진행한다.
3. 발견된 이슈를 수정한다:
   - 코드 수정 후 서버를 재빌드/재시작한다.
   - 수정 내용을 기록한다.
   - 이번 iteration에서 **실패한 각 케이스**에 대해, Step 2의 "실패→수정 블록 형식"으로 `/tmp/e2e-run-report.md`의 해당 케이스 블록 끝에 append 한다.
4. iteration 카운트를 1 증가시키고 1번으로 돌아간다.
5. **최대 5회** iteration 후에는 미해결 이슈와 함께 루프를 종료하고 Step 4로 진행한다.

| 종료 조건 | 결과 |
|----------|------|
| 모든 테스트 통과 (STATUS_MISMATCH 없음) | 루프 탈출 → Step 4 |
| 테스트 실패 0건 + 미커버만 존재 (`WARN`) | 루프 탈출 → Step 4, 미커버 사유를 상위에 전달 |
| e2e-test-mm가 `SKIPPED:*` 반환 | 루프 미진행, SKIPPED 그대로 보고 (Step 4 생략) |
| 5회 도달, 이슈 잔존 | 미해결 이슈 목록과 함께 Step 4로 강제 진행 |

### Step 4: E2E 리포트 HTML 렌더링 (정직한 자기 점검 형식 — v2)
...
> **건너뛰는 경우**: Step 1 Probe SKIP, `--init`, `--doctor`, 또는 Step 3에서 e2e-test-mm가 `SKIPPED:*`를 반환해 **테스트가 한 번도 실행되지 않은 경우**. 렌더링할 내용이 없으므로 이 단계를 생략한다. (e2e-test-mm가 1회 이상 정상 실행됐다면 통과/실패와 무관하게 항상 렌더링한다.)
...
3. 출력 경로를 결정한다: `/workspace/work-log/claude/{YYYYMMDD-HHMMSS}-{branch}-e2e-report.html`
   ...
   - **파일명 컨벤션은 고정**: 자산 프롬프트가 `-api-test-cases.html`을 권장하더라도, start-workflow-mm Phase 9.6/14가 `*-e2e-report.html` 패턴에 의존하므로 본 컨벤션을 우선한다.
...
> `E2E 리포트 HTML:` 줄은 **의무 출력**이다. 상위 워크플로우/오케스트레이터(start-workflow-mm Phase 9.6)가 이 경로를 **유일한 채널**로 전달받아 Phase 14 보고서에 참조한다. 서브 에이전트로 실행될 때도 이 줄이 stdout에 반드시 포함돼야 경로가 오케스트레이터까지 전파된다.
