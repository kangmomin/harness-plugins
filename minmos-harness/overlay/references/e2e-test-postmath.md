> 이 문서는 `minmos-harness` 오버레이의 `overlay/e2e-test.md` 에서 로드된다. 단독 실행 금지.
> `/be-harness:e2e-test` 베이스 절차 위에 얹는 **Post-Math 특화 E2E 절차 전문**이다.
...
1. **Apidog MCP 연결 확인**: `mcp__apidog__read_project_oas_*` 호출 가능 여부 확인. 불가하면 `/minmos-harness:apidog-schema-gen --init`과 동일한 Apidog 세팅을 안내한다.
...
- #1~#2 FAIL → 즉시 SKIP 반환 후 종료 (테스트 실패가 아닌 환경 미충족):
  ...
  사유: SKIPPED:{코드}
  누락 항목: {항목}
  복구 방법: /be-harness:e2e-test --init
  ```
...
코드베이스와 Apidog 스펙을 기반으로 엣지 케이스를 **독립적으로 도출**하고, 여기에 Spec이 명시한 케이스를 합집합으로 더한다.

**Spec 엣지 케이스 승계 (Spec에 `EC-*` ID가 있을 때):**
- `/be-harness:request` Phase 4가 만든 엣지 케이스 표(start-workflow에서 호출된 경우 상태 파일의 `## Edge Cases`)의 **각 행을 빠짐없이** 테스트 케이스로 포함한다.
- **ID는 Spec의 `EC-nn`을 그대로 승계한다** — 새 번호를 붙이거나 순서를 바꾸지 않는다. 아래 자체 도출 케이스는 `SELF-nn`으로 구분한다.
- 아래의 코드/Apidog 기반 자체 도출은 **그대로 유지한다.** Spec이 놓친 케이스를 잡는 독립 검증 축이므로, Spec 승계로 대체하지 않는다.
- 물리적으로 실측 불가능한 케이스(외부 서비스 장애 유발, 동시성 재현 불가, 시간 경과 필요 등)만 예외로 두고 `UNCOVERED:{사유}`로 리포트에 남긴다. **"검증이 번거롭다"는 예외 사유가 아니다.**
- Spec에 엣지 케이스 표가 없거나 ID가 없으면(구버전 Spec) 승계를 건너뛰고 자체 도출만 수행하며, 리포트 커버리지 섹션에 `대조 기준 없음`으로 표기한다.
