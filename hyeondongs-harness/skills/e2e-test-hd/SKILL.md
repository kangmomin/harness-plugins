...
4. **Spec 엣지 케이스 승계** (Spec에 `EC-*` ID가 있을 때): `/hyeondongs-harness:request-hd` Phase 4가 만든 엣지 케이스 표(start-workflow-hd에서 호출된 경우 상태 파일의 `## Edge Cases`)의 **각 행을 빠짐없이** 시나리오로 포함한다. 위 1~3의 자체 도출은 그대로 유지한다 — Spec이 놓친 흐름을 잡는 독립 축이다.
...
Step 4 리포트와 `start-workflow-hd` Phase 7.7 read-back이 이 ID로 커버리지를 대조하므로, ID를 바꾸거나 생략하지 않는다. 자체 도출 시나리오는 접두 없이 둔다.
