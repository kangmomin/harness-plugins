...
위 조건을 충족하지 않으면 `/hyeondongs-harness:request`를 호출하여 Technical Spec을 생성한다.
...
Agent tool:
  ...
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:simplify-loop 를 실행하세요.
...
Agent tool:
  subagent_type: general-purpose
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:convention-check 를 실행하세요.
    위반 사항이 있으면 수정하세요.
...
Agent tool:
  subagent_type: general-purpose
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:test-loop 를 실행하세요.
...
Agent tool:
  subagent_type: general-purpose
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:lint-check 를 실행하세요.
    이슈가 있으면 수정하세요.
    완료 후 "이슈: N건, 수정: Y/N" 형식으로 보고하세요.
```
...
```
Agent tool (병렬 1):
  subagent_type: hyeondongs-harness:component-reviewer
  prompt: |
