...
**전제 조건**: 서브 에이전트(workflow-implementer, scope-reviewer, component-reviewer, a11y-reviewer, workflow-pr, workflow-reflection)는 **fe-harness 플러그인**의 에이전트를 사용하므로 fe-harness 선행 설치가 필요하다.

**플레이스홀더 정의** (본문·references 공통, 값 변경은 여기 한 곳만 수정):

- `{STATE_FILE}` = `/tmp/workflow-state.md`
- `{CWD}` = 현재 작업 디렉토리 (프로젝트 루트)
- 빌드/타입 체크 명령 = `.hyeondong-config.json`의 `framework` 값에 따라 결정 (아래 Phase 6)
...
**기본**: `/hyeondongs-harness:request-hd`를 호출하여 Technical Spec을 생성한다. 완료 후 Spec 전문과 엣지 케이스 목록을 보관한다.
...
for iteration in 1..3:
  7.1 build + type-check      → Bash 직접 실행 (framework별 명령, 실패 시 수정 위임)
...
| `SKIPPED:{사유}` | 조건 미충족으로 건너뜀 (예: `SKIPPED:TASK_TYPE`) |
...
Phase 6: framework별 빌드+tsc 체크 (실패 시 수정 최대 3회)
