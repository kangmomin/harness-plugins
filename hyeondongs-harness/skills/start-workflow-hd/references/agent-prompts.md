> 이 문서는 `start-workflow-hd` 스킬의 Phase 5(구현), 6(빌드/타입 체크), 7(품질 루프), 8(리뷰), 9(PR), 10(성찰)에서 로드된다. 단독 실행 금지.
...
Agent tool:
  ...
  prompt: |
    프로젝트 루트 {CWD}에서 빌드/타입 체크 에러를 수정하세요 (framework별 명령은 SKILL.md Phase 6 참조).
...
Agent tool:
  ...
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:simplify-loop-hd 를 실행하세요.
...
Agent tool:
  ...
  effort: [품질 수정 범위 기준 선택]
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:convention-check-hd 를 실행하세요.
...
Agent tool:
  ...
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:test-loop-hd 를 실행하세요.
...
Agent tool:
  ...
  prompt: |
    프로젝트 루트 {CWD}에서 Skill tool로 /hyeondongs-harness:lint-check-hd 를 실행하세요.
