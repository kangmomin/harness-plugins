...
- 같은 문서의 work-log 쓰기는 읽기·해시 확인·쓰기 전체를 프로세스 간 직렬화한다.
  실제 문서 옆 `.{파일명}.write-lock`을 사용하므로 cache 경로나 vault 별칭이 달라도 적용된다.
  성공·예외 시 자기 락을 정리하며, 5초 대기 후에도 남아 있으면 소유자 정보와 오류를 반환한다.
  비정상 종료로 남은 락은 자동 회수하지 않는다. 소유 실행의 종료를 확인한 뒤 해당 락만 제거한다
