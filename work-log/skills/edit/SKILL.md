...
같은 문서의 work-log 쓰기는 프로세스 간 직렬화된다. 잠금 대기 시간 초과 시 오류에 나온 소유자 PID·호스트·시각을 확인한다. 실행이 살아 있으면 기다린다. 비정상 종료가 확인된 경우에만 오류에 표시된 `.{파일명}.write-lock` 디렉토리를 제거하고 다시 읽어 재시도한다. 소유자 정보가 없더라도 시간만으로 락을 제거하지 않는다.
