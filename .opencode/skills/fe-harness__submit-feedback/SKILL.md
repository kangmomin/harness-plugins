---
name: fe-harness__submit-feedback
description: "프로젝트 워크플로우에서 수집된 fe-harness 피드백을 플러그인 레포(kangmomin/harness-plugins)의 community-feedback 영역에 PR로 제출한다. 실패 시 로컬 저장으로 fallback."
...
- 피드백이 프로젝트 특정 도메인/스택에만 의미 있을 때 → 로컬 오버라이드(`.claude/fe-harness/...`)만 쓰고 여기 오지 않음
...
| 플러그인 `upstream.repo` | 필수 | `fe-harness/.claude-plugin/plugin.json` 읽기 |
| 네트워크 접근 | 필수 | 시도 후 판정 |
...
```bash
BRANCH="feedback/fe-harness/$(date +%Y%m%d)-{요약-kebab}"
git checkout -b "$BRANCH" 2>/dev/null || git checkout "$BRANCH"

# 각 feedback_item 에 대해 해당 파일에 append (없으면 생성)
# 파일 경로: fe-harness/community-feedback/{skills|agents|common}/{name}.md
...
```markdown
## Summary
fe-harness 사용 중 수집된 피드백 {N}건을 community-feedback 영역에 추가합니다.
...
자동 생성: /fe-harness:submit-feedback
```
...
fallback 시 호출자(`start-workflow`)는 로컬 저장(`.claude/fe-harness/...`)만 수행하고 정상 종료. 워크플로우 전체를 실패로 만들지 않는다.
...
> - Yes → PR 대상
> - No → 로컬 전용 (.claude/fe-harness/... 에만 저장, PR 생략)
