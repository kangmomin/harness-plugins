---
name: hyeondongs-harness__unit-test-hd
...
### 필요 환경
- **테스트 러너**: Vitest 또는 Jest (`.hyeondong-config.json`의 `testRunner` 참조)
...
1. `.hyeondong-config.json`의 `testRunner` 확인
...
| 항목 | 상태 | 비고 |
|------|------|------|
| .hyeondong-config.json | OK / MISSING | testRunner 설정 확인 |
...
`.hyeondong-config.json`의 `testRunner`에 따라 실행:
