---
name: minmos-harness__e2e-test-mm
...
프로젝트 루트의 `secret/.e2e-allowed-hosts` 파일에 호스트를 한 줄에 하나씩 등록하면 해당 호스트도 허용된다.
...
3. **사용자가 승인하면**:
   - `secret/.e2e-allowed-hosts` 파일에 해당 호스트를 추가한다 (파일이 없으면 생성).
   - 게이트 검증을 재실행하여 통과시킨다.
