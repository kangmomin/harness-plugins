---
name: hyeondongs-harness__convention-check-hd
...
3. **설정 파일 업데이트**: `.hyeondong-config.json`의 `conventions` 배열을 업데이트한다.
   ```json
   {
     "conventions": [
...
1. 프로젝트 루트에서 `.hyeondong-config.json`을 읽는다.
...
> "`UserList.tsx:25` — `useCallback` 의존성 배열에 `searchTerm`이 누락되어 있습니다. stale closure 위험."

#### 컴포넌트 구조 검사

`.hyeondong-config.json`의 `componentPattern`에 따라:
