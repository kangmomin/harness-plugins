...
Agent tool:  (× 2 — 백엔드 / 프론트엔드)
  ...
  prompt: |
    ...
    - git index 변경(add/reset/restore --staged)·커밋·공유 상태/노트 쓰기를 하지 마세요. 오케스트레이터가 배리어 후 처리합니다.
...
**완료 배리어**: 양쪽 에이전트와 쓰기 도구의 종료를 확인한 뒤 오케스트레이터만 상태/노트를 기록하고, 배정 범위의 diff를 검토·검증한 후 파일을 명시해 논리적 단위별로 순차 stage/commit한다. 다른 작업의 변경은 stage하지 않는다. 이 규칙은 Codex 위임/폴백에도 동일하다.
...
Agent tool:
  ...
  prompt: |
    ...
    ## 규칙
    - 프론트엔드 코드는 읽지 마세요.
    - 워크플로우 상태·구현 노트를 비롯한 명세·계약·계획 문서는 **읽지 마세요**. 존재하더라도 열지 마세요.
    - 파일을 수정하지 마세요.
