...
|------|------|------|
| Plan 검증 루프 | Phase 4.4 — effort: Simple·Standard → `xhigh`, Complex·Critical → `max` | — (Spec·계약·Plan 전문 전달) |
| 특화 하네스 품질 리뷰 | 해당 없음 (풀스택은 하위 워크플로우를 중첩 실행하지 않음) | — |
| 탐색·수집 / 이해·요약 | 8.1 계약 복원 에이전트 ×2 (general-purpose) | `{PLUGIN_ROOT}/skills/start-workflow/references/fullstack-agent-prompts.md` "계약 격리 Read-back 프롬프트 (Phase 8.1)" 절 |
| 읽기 전용 판정 | 3 계약 리뷰 advisor(Batch 1·2) · 8.2 통합 검증 리뷰어 | 3: `{PLUGIN_ROOT}/skills/start-workflow/references/contract-templates.md` "Phase 3" 절 / 8.2: `{PLUGIN_ROOT}/skills/start-workflow/references/fullstack.md` "Phase 8.2" 절 |
| 쓰기 | 6.1 BE/FE Red · 6.2 BE/FE 구현 · 7 도메인 품질 루프 수정(러너 내부 리프) · 10 `workflow-reflection` | `{PLUGIN_ROOT}/skills/start-workflow/references/fullstack-agent-prompts.md` 의 "Red 에이전트 프롬프트 (Phase 6.1)" / "구현 에이전트 프롬프트 (Phase 6.2)" 해당 절 (7·10은 general-purpose 취급) |
| 러너 대상 (항상 Claude — `max`면 §8 포인터 1줄) | 7 각 도메인 품질 스킬(`/be-harness:*` · `/fe-harness:*` Skill tool 실행) | — |
| PR (항상 Claude) | 9 커밋/PR | — |

- 풀스택 resolve: `--codex`는 common이 소비한다. 해석 = be profile → fe profile → 질문. 명시 입력은 존재하는 writable be·fe profile **모두**에 동일 값 기록 (부재·기록 실패 대상은 ephemeral 경고 1줄). 양쪽 값 상이 + 명시 입력 없음 → be 값을 **이번 실행만** 사용 + 고지 (기록 없음). 단일 도메인 위임은 `--codex`를 그대로 통과시킨다.
