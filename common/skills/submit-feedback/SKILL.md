...
> **Project Overrides**: 실행 전 `.claude/common/common.md`와 `.claude/common/skills/submit-feedback.md`를 Read.
> 존재하면 추가 규칙/예외로 흡수하고 충돌 시 오버라이드가 우선한다. 상세 규약: 플러그인 루트 `OVERRIDES.md`.

# submit-feedback — 피드백을 플러그인 레포에 PR로 제출

각 하네스의 워크플로우 최종 보고 Phase에서 수집된 **범용성 있는 보완점**을 플러그인 원본 레포의 `community-feedback/` 영역에 PR로 보낸다.

이 스킬은 **라우터가 아니다.** 하네스별 submit-feedback은 대상 플러그인 이름만 다르고 절차가 동일하므로 common이 직접 구현한다.
...
- 유저가 워크플로우 최종 보고 Phase에서 "로컬 저장 + PR" 옵션을 선택했을 때 자동 호출됨
  (Phase 번호는 하네스마다 다르다 — be/minmos 계열은 Phase 12·14, fe/hyeondongs 계열은 Phase 11)
- 수동으로 단독 실행도 가능 (`/common:submit-feedback`)

## 언제 쓰지 말아야 하나

- 피드백이 프로젝트 특정 도메인/스택에만 의미 있을 때 → 로컬 오버라이드(`.claude/{PLUGIN}/...`)만 쓰고 여기 오지 않음
...
| 플러그인 `upstream.repo` | 필수 | `{PLUGIN}/.claude-plugin/plugin.json` 읽기 |
| 네트워크 접근 | 필수 | 시도 후 판정 |
...
`{PLUGIN}`을 아래 우선순위로 정한다. **확정 전에는 어떤 파일도 쓰지 않는다.**
...
- 보완점이 **여러 플러그인에 걸쳐 있으면** 플러그인별로 나눠 **각각 별도 PR**로 제출한다. 하나의 PR에 섞지 않는다 (플러그인마다 리뷰 주체와 배포 주기가 다르다).
- 4순위에서 후보는 세션에 설치된 하네스로 한정한다.

### Step 2: 사전 조건 점검 + Dry-run 요약
...
- 모드: {self-owner / fork}
- 추가할 파일:
  - {PLUGIN}/community-feedback/skills/start-workflow.md (+1 엔트리)
  - {PLUGIN}/community-feedback/agents/workflow-implementer.md (+1 엔트리)
- 브랜치명: feedback/{PLUGIN}/{YYYYMMDD}-{요약-kebab}
- PR 제목: "[{PLUGIN}] feedback: {요약}"
...
### Step 5: 파일 append 및 커밋

```bash
BRANCH="feedback/{PLUGIN}/$(date +%Y%m%d)-{요약-kebab}"
git checkout -b "$BRANCH" 2>/dev/null || git checkout "$BRANCH"

# 각 feedback_item 에 대해 해당 파일에 append (없으면 생성)
# 파일 경로: {PLUGIN}/community-feedback/{skills|agents|common}/{name}.md
...
- 파일이 있으면 맨 아래에 Step 3 에서 작성한 섹션을 append
- 동일한 (날짜, target, summary) 조합이 이미 존재하면 중복이므로 건너뜀

커밋:

```bash
git add {PLUGIN}/community-feedback/
git commit -m "Add: {PLUGIN} 커뮤니티 피드백 추가: {요약}"
```
...
```markdown
## Summary
{PLUGIN} 사용 중 수집된 피드백 {N}건을 community-feedback 영역에 추가합니다.
...
## 출처
- 워크플로우: {PLUGIN} 워크플로우 성찰 Phase (자동 제출)

## 주의
- 원본 SKILL.md 는 변경하지 않습니다.
- 유지보수자 검토 후 범용 규칙으로 승격 여부를 결정합니다.

자동 생성: /common:submit-feedback
```

### Step 7: 결과 보고 + 임시 디렉토리 정리
...
fallback 시 호출자(`start-workflow`)는 로컬 저장(`.claude/{PLUGIN}/...`)만 수행하고 정상 종료. 워크플로우 전체를 실패로 만들지 않는다.
...
> - Yes → PR 대상
> - No → 로컬 전용 (.claude/{PLUGIN}/... 에만 저장, PR 생략)
