...
argument-hint: "[--be|--fe|--mm] <기능 설명 또는 요청>"
---
...
- 백엔드 계열(`--be`, `--mm`)과 프론트엔드 계열(`--fe`)은 **Spec 템플릿이 근본적으로 다르다** (엔드포인트·엣지 케이스 표 vs ASCII 레이아웃·Props). 후보에 양쪽이 모두 있으면 선택지에서 이 차이를 한 줄로 밝힌다.
