...
공통 플래그: `--reflect`(성찰 Phase 실행 — 기본 off), `--tier standard`(검증 티어 상향 강제 — 기본은 Spec 점수로 light/standard 자동 판정), `--hard`, `--no-tdd`. 워크플로우 종료 시 md Workflow Report가 profile `reportDir`에 아카이브된다.
